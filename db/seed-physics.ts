// @ts-nocheck
/**
 * Seeds Physics practice data from ACT Crack (Shahd Gaber).
 * 36 passages, 274 questions with full explanations, plus figure/table
 * images extracted directly from the source PDF and matched to passages by
 * page range + marker order.
 *
 * Usage: tsx db/seed-physics.ts
 */
...
const passages: Array<{
  ...
  readFileSync(join(__dirname, "practice-physics.json"), "utf-8"),
);
...
> = JSON.parse(
  readFileSync(join(__dirname, "practice-physics-images.json"), "utf-8"),
);

const db = getDb();

async function main() {
  console.log(`🌱  Seeding Physics: ${passages.length} passages...`);
  let totalImages = 0;
  ...
  console.log(`✅  Physics seed complete — ${passages.length} passages, ${passages.reduce((s, p) => s + p.questions.length, 0)} questions, ${totalImages} figure images.`);
  process.exit(0);
}

main().catch((e) => {
  console.error("Physics seed failed:", e);
  process.exit(1);
});
